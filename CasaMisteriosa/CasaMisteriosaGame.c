...
void InitGameData() {
    InitHabitacion(&habitaciones[1], 0, 4, 0, 0, "Estas en una pequenia habitacion pintada de blanco. \nJunto a ti puedes ver una cama y un velador con un oso de peluche viejo encima.");
    InitHabitacion(&habitaciones[2], 0, 5, 0, 0, "Estas en una habitacion pintada de verde. \nJunto a ti puedes ver una cama grande y un velador con fotos de una familia.");
    InitHabitacion(&habitaciones[3], 0, 6, 0, 0, "Estas en el comedor de la casa. hay una mesa rodeada de sillas.");
    ...
    InitHabitacion(&habitaciones[6], 3, 9, 0, 5, "Estas en un pasillo, ves muchos cuadros y libros.");    InitHabitacion(&habitaciones[7], 4, 0, 0, 0, "Estas en el banio, el lavamanos esta desordenado y el inodoro es cafe.");
...
int main(int argc, char** argv) {
    ...
    while (!juegoTerminado) {
        ...
        if (!strcmp(verbo, "tomar")) {
            ...
            // Llave
            if (!strcmp(nombreObjeto, "llave") && objetos[LLAVE].habitacion == habitacionActual) {
        ...
        if (!strcmp(verbo, "usar")) {
            ...
            if (!strcmp(nombreObjeto, "llave") && objetos[LLAVE].yaLoTengo && habitacionActual == 8) {
                accion = 1;
                printf("\n((Sientes una brisa en tu espalda, giras la llave y abres la puerta, afuera esta de noche, hay viento y hace frio. Pero haz logrado escapar, felicidades))");
